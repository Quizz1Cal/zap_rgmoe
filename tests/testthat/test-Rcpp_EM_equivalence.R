if (T) {
    ...
    test_that("EM_run matches (masked, Proximal)", {
        ...
        data_cpp <- EM_run(data$Zs, data$is_masked, data$X_f, data, data,
                           use_proximal_newton=T, use_cpp=T, verbose=F)
        data_R <- EM_run(data$Zs, data$is_masked, data$X_f, data, data,
    ...
    test_that("EM_run matches (masked, Proximal-Type)", {
        ...
        expect_equal(data_R, data_cpp, tolerance=1e-9)
    })
