...
R_gating_update <- function(X_f, tau, w_f, gamma, use_proximal_newton=FALSE) {
    n = dim(X_f)[1]
    K = dim(tau)[2]
    step_size = 0.5
    eps = 1e-5
    ...
    repeat {
        w_f_old = w_f_new
        Q_old = R_Fs(X_f, tau, w_f_old, gamma)
        for(k in 1:(K-1)) {
            #First: compute the quadratic approximation w.r.t (w_k): L_Qk
            P_k = R_pi_matrix(X_f, w_f_new)[,k]
            if (use_proximal_newton) {
                d_k = P_k*(1-P_k)
            ...
            w_f_new[,k] <- R_weight_marginal_CD(Y=c_k, X_f=X_f, tau=d_k,
                                              w_f_new[,k], gamma[k])
        }
        Q_new = R_Fs(X_f, tau, w_f_new, gamma)
...
R_weight_marginal_CD <- function(Y, X_f, tau, wk_f, gammak) {
    stopifnot(all(tau >= 0) & all(tau <= 1))
    eps = 1e-6
    p = dim(X_f)[2] - 1
    cur_val = R_obj_gating(Y, X_f, tau, wk_f, gammak)
    repeat {
        prev_val = cur_val
        for(j in 2:(p+1)) {
            rj = Y - X_f[,-j]%*%wk_f[-j]
            ...
            wk_f[j] = R_SoTh(numerator, gammak)/denominator
...
R_obj_gating <- function(Y, X_f, tau, wk_f, gammak) {
