beta_update <- function(X_f, D0, D1, D2, beta_f, sigma2, lambda) {
    ...
    # Compute in series
    for (k in 1:K) {
        beta_f[,k] <- beta_marginal_CD(X_f, D0[,k], D1[,k], D2[,k], beta_f[,k],
                                       sigma2[k], lambda[k])
...
beta_marginal_CD <- function(X_f, D0k, D1k, D2k, betak_f, sigma2_k, lambda_k) {
    # Inspired by weight_marginal_CD. Note that they are all k-specific.
    eps = 1e-6
    p = dim(X_f)[2] - 1
    cur_val = obj_expert(X_f, D0k, D1k, D2k, betak_f, sigma2_k, lambda_k)
    ...
    repeat {
        ...
        for (j in 2:(p+1)) {
            ...
            denom <- D0k %*% (X_f[,j]^2)
            betak_f[j] <- SoTh(X_f[,j] %*% rj, sigma2_k*lambda_k) / denom
